...
import { get, omit, flattenDepth, unionBy } from 'lodash';
...
const menusFromEntries = (entries: Entry[]) => {
  ...
  return unionBy(menus, entriesWithoutMenu, 'name');
};
...
const normalize = (item: MenuItem | string): MenuItem => {
  ...
  return {
    ...selected,
    id: selected.id || ulid(),
    parent: get(selected, 'parent') || get(item, 'parent'),
...
const clean = (item: MenuItem) =>
  item.href || item.route ? omit(item, 'menu') : item;

const normalizeAndClean = (item: MenuItem) => {
  const normalized = normalize(item);
  const cleanedAndNormalized = clean(normalized);
...
const mergeMenus = (entriesMenu: Menus, configMenu: Menus): Menus => {
  ...
  const merged = unionBy(first, second, 'name') as MenuItem[];
...
const sortMenus = (first: Menus, second: Menus | undefined = []): Menus => {
  const sorted: Menus = sort<MenuItem>(
    first,
    compareWithMenu<MenuItem>(second),
    //@ts-ignore TODO: remove sortByName, sort method expects object as 3rd argument not comparison method.
    sortByName
  );
